...
# --- Preprocesamiento AUTOMÁTICO (Loop) ---
# Creamos una lista vacía para guardar los inputs procesados
inputs_procesados = []

for index, row in candidatos.iterrows():
    # 1. Calcular lógica difusa para la edad
    fuzzy_vals = obtener_fuzzy_edad(row['edad'], x_edad)
    
    # 2. Escalar el salario (usando el mismo scaler del entrenamiento)
    salario_sc = scaler.transform([[row['salario']]])[0][0]
    
    # 3. Guardar vector [Salario, Joven, Adulto, Mayor]
    inputs_procesados.append([salario_sc, fuzzy_vals[0], fuzzy_vals[1], fuzzy_vals[2]])

# Convertimos a numpy array para que TensorFlow lo entienda
X_nuevos = np.array(inputs_procesados)
...
for i, pred in enumerate(predicciones):
    decision = "SI Compra" if pred > 0.5 else "NO Compra"
    print(f"{candidatos.iloc[i]['nombre']}: Probabilidad {pred[0]*100:.1f}% -> {decision}")